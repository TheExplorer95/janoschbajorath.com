...
- Structure: hero clip above the abstract · Abstract (the poster's Vision + Problem) · Contributions (three couplings, linked to their sections) · Setup (architecture figure, parameters, gait table, note on reading the clips) · one `##` section per coupling with one `###` per hypothesis (claim, clip, figure, caption) · Conclusion (stage one, stage two) · References.
...
- `<Clip name? id? title wide?>caption</Clip>`: `name` plays the self-hosted clip listed in `src/data/where-2026-clips.json` (rendered by `tools/clips/`, see `2026-09-21-where-2026-clips-design.md`), `id` the YouTube embed, neither the placeholder. Slots follow gait_rl's `config/eval/video_*.py`: capabilities (hero), gaits (H1 Behaviour), domain (H2 Domain), jump (H2 Latent Feedback), push (H1 Robustness).
